...
TEST() {
  ...
  const jasTaggedOperand_t registerOperand = jasConstructOperand((void *)&argContent, JAS_REG_OPERAND_16);
  ...
  SHOULD_EQUAL(realModeInstance.buffer[0], 0x63);
  SHOULD_EQUAL(realModeInstance.buffer[1], 0xC0);
