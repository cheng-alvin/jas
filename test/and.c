...
TEST() {
  ...
  ({
    ...
    const jasErrorCode_t status = jasCodegen(AND, al, imm8, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);

    SHOULD_EQUAL(status, JAS_NO_ERROR);
    SHOULD_EQUAL(realModeInstance.buffer[0], 0x24);
    SHOULD_EQUAL(realModeInstance.buffer[1], 0xFF);

    const jasErrorCode_t status2 = jasCodegen(AND, ax, imm16, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);

    SHOULD_EQUAL(status2, JAS_NO_ERROR);
    SHOULD_EQUAL(realModeInstance.buffer[2], 0x66);
    SHOULD_EQUAL(realModeInstance.buffer[3], 0x25);
    SHOULD_EQUAL(realModeInstance.buffer[4], 0xFF);
    SHOULD_EQUAL(realModeInstance.buffer[5], 0xFF);

    const jasErrorCode_t status3 = jasCodegen(AND, eax, imm32, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);

    SHOULD_EQUAL(status3, JAS_NO_ERROR);
    SHOULD_EQUAL(realModeInstance.buffer[6], 0x25);
    ...
    const jasErrorCode_t status4 = jasCodegen(AND, rax, imm32, JAS_NO_OPERAND, JAS_NO_OPERAND, &tempRealModeInstance);

    SHOULD_EQUAL(status4, JAS_LONG_MODE_INSTRUCTION);

    const jasErrorCode_t status5 = jasCodegen(AND, rax, imm32, JAS_NO_OPERAND, JAS_NO_OPERAND, &longModeInstance);

    SHOULD_EQUAL(status5, JAS_NO_ERROR);
    SHOULD_EQUAL(longModeInstance.buffer[0], 0x48);
    SHOULD_EQUAL(longModeInstance.buffer[1], 0x25);
  ...
  ({
    ...
    const jasErrorCode_t status = jasCodegen(AND, cl, imm8, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);

    SHOULD_EQUAL(status, JAS_NO_ERROR);
    SHOULD_EQUAL(realModeInstance.buffer[11], 0x80);
    SHOULD_EQUAL(realModeInstance.buffer[12], 0xE1);
    SHOULD_EQUAL(realModeInstance.buffer[13], 0xFF);

    const jasErrorCode_t status2 = jasCodegen(AND, bx, imm16, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);

    SHOULD_EQUAL(status2, JAS_NO_ERROR);
    SHOULD_EQUAL(realModeInstance.buffer[14], 0x66);
    SHOULD_EQUAL(realModeInstance.buffer[15], 0x81);
    SHOULD_EQUAL(realModeInstance.buffer[16], 0xE3);
    SHOULD_EQUAL(realModeInstance.buffer[17], 0xFF);
    SHOULD_EQUAL(realModeInstance.buffer[18], 0xFF);

    const jasErrorCode_t status3 = jasCodegen(AND, ebx, imm32, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);

    SHOULD_EQUAL(status3, JAS_NO_ERROR);
    SHOULD_EQUAL(realModeInstance.buffer[19], 0x81);
    SHOULD_EQUAL(realModeInstance.buffer[20], 0xE3);
    ...
    const jasErrorCode_t status4 = jasCodegen(AND, rbx, imm32, JAS_NO_OPERAND, JAS_NO_OPERAND, &tempRealModeInstance);

    SHOULD_EQUAL(status4, JAS_LONG_MODE_INSTRUCTION);

    const jasErrorCode_t status5 = jasCodegen(AND, rbx, imm32, JAS_NO_OPERAND, JAS_NO_OPERAND, &longModeInstance);

    SHOULD_EQUAL(status5, JAS_NO_ERROR);
    SHOULD_EQUAL(longModeInstance.buffer[6], 0x48);
    SHOULD_EQUAL(longModeInstance.buffer[7], 0x81);
    SHOULD_EQUAL(longModeInstance.buffer[8], 0xE3);
  ...
  ({
    ...
    const jasErrorCode_t status2 = jasCodegen(AND, bx, imm8, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);

    SHOULD_EQUAL(status2, JAS_NO_ERROR);
    SHOULD_EQUAL(realModeInstance.buffer[25], 0x66);
    SHOULD_EQUAL(realModeInstance.buffer[26], 0x83);
    SHOULD_EQUAL(realModeInstance.buffer[27], 0xE3);
    SHOULD_EQUAL(realModeInstance.buffer[28], 0xFF);

    const jasErrorCode_t status3 = jasCodegen(AND, ebx, imm8, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);

    SHOULD_EQUAL(status3, JAS_NO_ERROR);
    SHOULD_EQUAL(realModeInstance.buffer[29], 0x83);
    SHOULD_EQUAL(realModeInstance.buffer[30], 0xE3);
    SHOULD_EQUAL(realModeInstance.buffer[31], 0xFF);

    const jasErrorCode_t status4 = jasCodegen(AND, rbx, imm8, JAS_NO_OPERAND, JAS_NO_OPERAND, &tempRealModeInstance);

    SHOULD_EQUAL(status4, JAS_LONG_MODE_INSTRUCTION);

    const jasErrorCode_t status5 = jasCodegen(AND, rbx, imm8, JAS_NO_OPERAND, JAS_NO_OPERAND, &longModeInstance);

    SHOULD_EQUAL(status5, JAS_NO_ERROR);
    SHOULD_EQUAL(longModeInstance.buffer[13], 0x48);
    SHOULD_EQUAL(longModeInstance.buffer[14], 0x83);
    SHOULD_EQUAL(longModeInstance.buffer[15], 0xE3);
  ...
  ({
    ...
    const jasErrorCode_t status = jasCodegen(AND, cl, cl, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);
    ...
    SHOULD_EQUAL(realModeInstance.buffer[35], 0x23)
    SHOULD_EQUAL(realModeInstance.buffer[36], 0xDB)

    const jasErrorCode_t status3 = jasCodegen(AND, ebx, ebx, JAS_NO_OPERAND, JAS_NO_OPERAND, &realModeInstance);

    SHOULD_EQUAL(status, JAS_NO_ERROR)
    SHOULD_EQUAL(realModeInstance.buffer[37], 0x23)
    SHOULD_EQUAL(realModeInstance.buffer[38], 0xDB)

    const jasErrorCode_t status4 = jasCodegen(AND, rbx, rbx, JAS_NO_OPERAND, JAS_NO_OPERAND, &tempRealModeInstance);

    SHOULD_EQUAL(status4, JAS_LONG_MODE_INSTRUCTION);

    const jasErrorCode_t status5 = jasCodegen(AND, rbx, rbx, JAS_NO_OPERAND, JAS_NO_OPERAND, &longModeInstance);

    SHOULD_EQUAL(status5, JAS_NO_ERROR);
    SHOULD_EQUAL(longModeInstance.buffer[17], 0x48);
    SHOULD_EQUAL(longModeInstance.buffer[18], 0x23);
