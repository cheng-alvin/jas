...
TEST(jasAAA) {
  ...
  jasErrorCode_t aaaExitStatus = jasAAA(JAS_NO_OPERAND, JAS_NO_OPERAND, JAS_NO_OPERAND, JAS_NO_OPERAND, instance);
  SHOULD_EQUAL(aaaExitStatus, JAS_NO_ERROR)
  SHOULD_EQUAL(buffer[0], 0x37)

  instance->mode = JAS_MODE_64;
  aaaExitStatus = jasAAA(JAS_NO_OPERAND, JAS_NO_OPERAND, JAS_NO_OPERAND, JAS_NO_OPERAND, instance);
